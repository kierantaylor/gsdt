...
describe('Fetch data using Exercise Service', () => {
	it('fetches successfully data from an API', async () => {
		...
		const mockAddListener = jest.spyOn(axios, 'get') // spy on foo.addListener
		mockAddListener.mockImplementationOnce(() => Promise.resolve(data))

		await expect(getExercises()).resolves.toEqual(data)
	})

	it('fetches bad data from an API', async () => {
		...
		await expect(getExercises()).rejects.toThrow(errorMessage)
	})
})
